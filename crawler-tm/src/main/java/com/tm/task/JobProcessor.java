...
public class JobProcessor implements PageProcessor {
    ...
    private String url = "https://search.yhd.com/c0-0/k%25E6%2589%258B%25E6%259C%25BA/#&sort=1&page=";
    ...
    public void process(Page page) {
        //解析页面，获取商品列表的url地址
        List<Selectable> list = page.getHtml().css("div#itemSearchList").nodes();
        ...
        if (list.size() == 0) {
            ...
            for (Selectable selectable : list) {
                ...
                //获取url地址
                String url = selectable.css("div#searchProImg").links().toString();
                String image = selectable.css("div#searchProImg img","src").toString();
                String str = selectable.css("p.proName").toString();
                String name = Jsoup.parse(str).select("a").attr("title");
                String priceStr = Jsoup.parse(selectable.css("p.proPrice").toString()).select("em").text();
                double price = Double.parseDouble(priceStr.substring(1,priceStr.length()));

                goods.setName(name);
                goods.setPrice_before(price);
                goods.setUrl(url);
                goods.setImage(image);
                page.putField("goods", goods);

                //把获取到的url地址放到任务队列中
                //page.addTargetRequest(goodsUrl);

            }

            //获取下一页的url
            pageNum = pageNum + 1;
            String bkUrl = url + pageNum;
            //把url放到任务队列中
